...
class dbobject extends validation
{
	...
	public function regenerateCORS($data)
	{

		$key = date('Y-m-d h:m:s');
		$data = base64_encode(hash("sha256", $key, true));


		$_SESSION['eagle_cors_token_sess'] = $data;

		return $_SESSION['eagle_cors_token_sess'];
	}
...
*/
	...
	function getitemcountAdmin($tablename, $table_col, $table_val, $ret_val)
	{
		...
		$query = "select Count(" . $ret_val . ") counter from " . $tablename ;
		//echo $query;
	...
	function getTableRows($tablename,$table_col,$table_val,$count=0) {
		//$count =0 Mean All no limit 
		 $limit="";
		 if($count>0){
			$limit=" LIMIT ".$count;
		 }
		$lab=array();
		$table_filter = " where ".$table_col."='".$table_val."'";
		$query = "select * from ".$tablename.$table_filter.$limit;
		//echo $query;
	...
	public function cleanUp($array)
	{
		...
		foreach ($array as $key => $value) {
			if (is_array($value)) {
				$array[$key] = $this->cleanUp($value);
			} else {
				$array[$key] = mysqli_real_escape_string($this->myconn = $cnx->connect(), trim($value));
			}
		}
		return $array;
	}
	...
	public function sendMailEmailNotifications($to, $subject, $template_file)
    {
        ...
        try {
            ...
            $mail->Username   = 'ezekialafolabi11@gmail.com';                     //SMTP username
            $mail->Password   = 'sqhbzlunbcyfqvyj';                               //SMTP password
            ...
            //Recipients
            $mail->setFrom('ezekialafolabi11@gmail.com', 'ANPR');
            ...
            //Content
            $mail->isHTML(true);
			// $mail->AddEmbeddedImage('assets/images/evreg2.png', 'logo');                                  //Set email format to HTML
